...
with tf.Session() as sess:
    ...
    ###
    print("Accuracy (with no injections):", accuracy.eval({X: batch_xs[:150], Y: batch_ys[:150]}))
    orgAcy = accuracy.eval({X: batch_xs[:150], Y: batch_ys[:150]})
    fi.turnOnInjections()	
    print("Accuracy (with injections):", accuracy.eval({X: batch_xs[150:], Y: batch_ys[150:]}))
