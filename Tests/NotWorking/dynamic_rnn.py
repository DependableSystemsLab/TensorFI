...
with tf.Session() as sess:
    ...
    fi = ti.TensorFI(sess, name = "Dynamic", logLevel = 10, disableInjections = True)

    # Print the graph to the console for debugging
    # fi.printGraph()	
    ...
    # Calculate accuracy for 128 mnist test images (again after instrumenting)
    correctResult = sess.run(accuracy, feed_dict={x: test_data, y: test_label,
                                      seqlen: test_seqlen})
    ...
    myStats = ti.FIStat("Recurrent")	
    fi.launch( numInjections = 100, computeDiff = diffFunc, collectStats = myStats) 	
